...
class AyeCode_Connect_Turnstile {
	...
	public function __construct() {
		...
		$defaults = array(
			...
			'protections'   => array(
                ...
                'uwp_newsletter_subscribe'       => 1,
                'uwp_newsletter_unsubscribe'      => 1,
                'uwp_cm_subscribe'       => 1,
                'uwp_cm_unsubscribe'      => 1
			)
		);
	...
	private function init_hooks() {
		...
		if ( $this->get_site_key() && $this->get_secret_key() ) {
			if ( $pagenow && $pagenow == 'admin.php' && isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'ayecode-turnstile' ) {
                ...
                if ( ! empty( $this->options['protections']['uwp_newsletter_subscribe'] ) || ! empty( $this->options['protections']['uwp_newsletter_unsubscribe'] ) ) {
                    add_action( 'uwp_newsletter_subscribe_fields', array( $this, 'add_turnstile_uwp_newsletter_forms' ), 10, 1 );
                    add_action( 'uwp_newsletter_form_validate', array( $this, 'verify_uwp_newsletter_subscribe' ), 20,1 );
                }

                if ( ! empty( $this->options['protections']['uwp_cm_subscribe'] ) || ! empty( $this->options['protections']['uwp_cm_unsubscribe'] ) ) {
                    add_action( 'uwp_cm_subscribe_fields', array( $this, 'add_turnstile_uwp_cm_forms' ), 10, 1 );
                    add_action( 'uwp_cm_form_validate', array( $this, 'verify_uwp_cm_subscribe' ), 20,1 );
                }
    ...
    public function add_turnstile_uwp_newsletter_forms( $args ) {
        $ayecode_turnstile_options = get_option( 'ayecode_turnstile_options');
        if ( $args['type'] == 'subscribe' && ! empty($ayecode_turnstile_options['protections']['uwp_newsletter_subscribe'])) {
            $this->add_turnstile_widget();
        }

        if ($args['type'] == 'unsubscribe' && ! empty($ayecode_turnstile_options['protections']['uwp_newsletter_unsubscribe']) ) {
            $this->add_turnstile_widget();
        }
    }

    public function verify_uwp_newsletter_subscribe($data) {
        $ayecode_turnstile_options = get_option( 'ayecode_turnstile_options');
        if(is_array($data)) {
            if($data['action'] == 'uwp_newsletter_subscribe' && $ayecode_turnstile_options['protections']['uwp_newsletter_subscribe'] == true) {
                $verify = $this->verify_turnstile( 'uwp_newsletter_subscribe' );
                if ( is_wp_error( $verify ) ) {
                    return $verify;
                }
            }
            if($data['action'] == 'uwp_newsletter_unsubscribe' && $ayecode_turnstile_options['protections']['uwp_newsletter_unsubscribe'] == true) {
                $verify = $this->verify_turnstile( 'uwp_newsletter_unsubscribe' );
                if ( is_wp_error( $verify ) ) {
                    return $verify;
                }
            }
	...
	public function add_turnstile_uwp_cm_forms( $args ) {
        $ayecode_turnstile_options = get_option( 'ayecode_turnstile_options');
        if ( $args['type'] == 'subscribe' && ! empty($ayecode_turnstile_options['protections']['uwp_cm_subscribe'])) {
            $this->add_turnstile_widget();
        }

        if ($args['type'] == 'unsubscribe' && ! empty($ayecode_turnstile_options['protections']['uwp_cm_unsubscribe']) ) {
            $this->add_turnstile_widget();
        }
    }

    public function verify_uwp_cm_subscribe($data) {
        $ayecode_turnstile_options = get_option( 'ayecode_turnstile_options');
        if(is_array($data)) {
            if($data['action'] == 'uwp_cm_subscribe' && $ayecode_turnstile_options['protections']['uwp_cm_subscribe'] == true) {
                $verify = $this->verify_turnstile( 'uwp_cm_subscribe' );
                if ( is_wp_error( $verify ) ) {
                    return $verify;
                }
            }
            if($data['action'] == 'uwp_cm_unsubscribe' && $ayecode_turnstile_options['protections']['uwp_cm_unsubscribe'] == true) {
                $verify = $this->verify_turnstile( 'uwp_cm_unsubscribe' );
                if ( is_wp_error( $verify ) ) {
                    return $verify;
                }
            }
	 ...
	 *
	 * @param string $context Which form the widget is for, where a call site names one.
	 *
	...
	public function add_turnstile_widget( $context = '' ) {
		...
		// Hooks pass their own arguments here, so only accept a deliberate string.
	 ...
	 * @param $attempt  What is being attempted, where one verification spans two requests.
	 *
	...
	private function verify_turnstile( $context = '', $is_admin = false, $attempt = '' ) {
		...
		// Wordfence runs the authenticate chain twice per login; one key, one ruling.
		if ( $attempt !== '' ) {
			$asking['idempotency_key'] = $this->get_idempotency_key( $token, $context, $attempt );
		}
		...
		$said = json_decode( wp_remote_retrieve_body( $response ), true );

		if ( ! is_array( $said ) || empty( $said['success'] ) ) {
			return new WP_Error(
				'turnstile_failed',
		...
		// Stops a token solved on one form being spent on another.
		if ( $context !== '' && isset( $said['action'] ) && $said['action'] !== '' && $said['action'] !== $context ) {
			return new WP_Error(
		...
		if ( ! empty( $said['hostname'] ) && ! empty( $host ) && $said['hostname'] !== $host ) {
			/**
			 * Filter the hostnames accepted by Turnstile verification.
			 *
			 * @since 1.4.22
			 *
			 * @param array  $hosts   Accepted hostnames.
			 * @param string $context Which form asked.
			 */
			$allowed = apply_filters( 'ayecode_turnstile_allowed_hostnames', array( $host ), $context );

			if ( ! in_array( $said['hostname'], (array) $allowed, true ) ) {
				return new WP_Error(
					'turnstile_wrong_host',
					__( 'Security verification failed.', 'ayecode-connect' )
				);
			}
		}
	 ...
	 *
	 * Bound to what is being attempted, so a replayed token with different credentials
	 * derives a different key and is refused as already spent.
	 *
	 * @since 1.4.22
	 *
	 ...
	 *
	 * @return string A v4 UUID, as Cloudflare requires.
	 */
	 ...
	 *
	 * The form is left holding a token its AJAX has already spent.
	 *
	 * @since 1.4.22
	 *
	 * @return void
